...
from snake.cellgraph import COLORS, CellGraph
...
def execute(args):
    ...
    snake = Snake(
        args.width, args.height, colors, food=args.food, filename=args.filename
    )
...
Los colores disponibles son:
    ...
    parser.add_argument(
        ...
        default="gameoflife",
        dest="name",
        help="""nombre con el que se guarda la
                        captura de pantalla(si se hace)""",
    )
    parser.add_argument(
        "-w", "--width", type=int, default=30, help="numero de celdas horizontales"
    )
    parser.add_argument(
        "--height", type=int, default=30, help="numero de celdas verticales"
    )
    parser.add_argument(
        "-mw",
        "--margin-width",
    ...
    parser.add_argument(
        "-ch", "--cell-height", type=int, default=5, help="ancho vertical de las celdas"
    )
    parser.add_argument(
        "-sbc",
    ...
    parser.add_argument(
        "-f", "--food", type=int, default=1, help="cantidad de comida en el tablero"
    )
    parser.add_argument(
        "-bc",
        "--background-color",
    ...
    parser.add_argument(
        "-v", "--velocity", type=int, default=10, help="""velocidad de la culebrita"""
    )
    parser.add_argument(
        "-a",
        "--acceleration",
        type=float,
        default=0,
        help="""aceleracion de la culebrita""",
    )
