    ...
    def clear(self, key, matrix=None):
        ...
        if key == "c":
            if not matrix:
                ...
                self.matrix = [[self.nullCell] * self.width for i in range(self.height)]
    ...
    def draw(self, screen):
        ...
        for i in range(self.system.height):
            for j in range(self.system.width):
                p.draw.rect(
                    screen,
                    COLORS[self.system.getColor(i, j)],
                    p.Rect(
                        self.margin_width
                        + j * self.cellwidth
    ...
    def eventManager(self, screen):
        ...
        for event in setEvents():
            ...
            if event.type == p.MOUSEBUTTONDOWN:
                self.system.mouseButtonDown(self.getPositionInMatrix(event.pos), event)
                self.reload(screen)
            if event.type == p.KEYUP:
                self.system.keyUp(p.key.name(event.key))
                self.reload(screen)
            if event.type == p.MOUSEBUTTONUP:
                self.system.mouseButtonUp(self.getPositionInMatrix(event.pos), event)
                self.reload(screen)
